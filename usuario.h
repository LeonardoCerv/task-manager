...
void Usuario::crearTarea() {
  ...
  if (tipo == "Trabajo" || tipo == "trabajo") { 
    ...
    tareas.push_back(t);
    cout<<"Se ha creado la tarea "<<tareas.back()->getNombre()<<endl;
  } 
...
void Usuario::mostrarTareasLista(Lista lista) {
  ...
  for (Tarea * j: lista.getTareas()){
    for (int i = 0; i < tareas.size(); i++){
      if (tareas[i]->getNombre() == j->getNombre() && tareas[i]->
        getDescripcion() == j->getDescripcion()){
        valid = true;
        }
      }
      if (!valid){
        cout<<"La tarea "<<j->getNombre()<<" no existe en la lista de tareas"
            <<endl;
  ...
  for (Tarea * i: lista.getTareas()) {
    ...
    cout<<"Descripcion: "<<i->getDescripcion()<<endl;
  }
}
